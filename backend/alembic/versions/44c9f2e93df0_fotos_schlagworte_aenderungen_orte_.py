"""Fotos, Schlagworte, Aenderungen, Orte, Importprotokoll

Revision ID: 44c9f2e93df0
Revises: a510bd1a68f4
Create Date: 2026-07-28 01:28:58.640316
"""
...
revision: str = "44c9f2e93df0"
down_revision: str | None = "a510bd1a68f4"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("(lat IS NULL) = (lon IS NULL)", name="ck_koordinatenpaar"),
        sa.CheckConstraint("date_to IS NULL OR date_from <= date_to", name="ck_zeitraum"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("sha256"),
    )
    with op.batch_alter_table("photos", schema=None) as batch_op:
        batch_op.create_index("ix_photos_ort", ["lat", "lon"], unique=False)
        batch_op.create_index("ix_photos_status", ["status"], unique=False)
        batch_op.create_index("ix_photos_zeit", ["date_from", "date_to"], unique=False)
    ...
    with op.batch_alter_table("places", schema=None) as batch_op:
        batch_op.create_index("ix_places_suche", ["name_normalized"], unique=False)
    ...
    with op.batch_alter_table("changes", schema=None) as batch_op:
        ...
        batch_op.create_index("ix_changes_zeit", ["created_at"], unique=False)
    ...
    with op.batch_alter_table("import_log", schema=None) as batch_op:
        batch_op.create_index("ix_import_log_zeit", ["created_at"], unique=False)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("import_log", schema=None) as batch_op:
        batch_op.drop_index("ix_import_log_zeit")

    op.drop_table("import_log")
    with op.batch_alter_table("changes", schema=None) as batch_op:
        batch_op.drop_index("ix_changes_zeit")
        batch_op.drop_index("ix_changes_photo")

    op.drop_table("changes")
    op.drop_table("tags")
    with op.batch_alter_table("places", schema=None) as batch_op:
        batch_op.drop_index("ix_places_suche")

    op.drop_table("places")
    with op.batch_alter_table("photos", schema=None) as batch_op:
        batch_op.drop_index("ix_photos_zeit")
        batch_op.drop_index("ix_photos_status")
        batch_op.drop_index("ix_photos_ort")
